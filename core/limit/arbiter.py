...
- 每一轮：
  - 当前候选者尝试贴出 124
  - 所有 Bot 在固定窗口内观测是否出现 124
- 首个成功确认 124 的候选者成为实际胜出者

若所有候选者均未贴出 124，则本轮仲裁作废。

──────────────────────────────────────────────────────────────────────
五·附：单参与者快速路径（Fast-Path）
──────────────────────────────────────────────────────────────────────
        ...
        for candidate in order:
            ...
            if candidate == ctx.self_id:
                try:
                    ...
                    logger.warning(
                        f"[arbiter][feedback] set 124 failed: {e}", exc_info=True
                    )
